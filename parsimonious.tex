...
Bayesian model selection for binary outcomes: unlike regularization, this is a model search
through the sampling of predictors with MCMC algorithm that builds upon
the conjugacy structure of data and parameters.

BOSS: Bayesian stochastic model search for binary outcomes. The
relationships between binary observed responses and predictors is
described by a altent variable model with a probit link. Prior distributions assigned
...
$Y_i$: vector of $i=1 ... n$ observations; independent Bernoulli trials
with success probability $\pi_i$. $x_i=(x_{i1}, \ldots, x_{ip})$ vector
of $p$ predictors associated with response $Y_i$. $\beta$: vector of
regression coefficients (parameters).

Probit link function $g$:

\begin{equation}
\pi_{i}=\phi(x_i^T \beta) = \int_{-\infty}^{x_i^T \beta}N(0,1)\mathrm{d}u
\end{equation} 
...
$\gamma=(\gamma_1, \ldots, \gamma_p)$: vector of indicator variables
($\gamma=0$ if $\beta=0$, $\gamma=1$ if $\beta \neq 0$)
